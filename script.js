...
function estimateTimeToGoal(currentKg, goalKg) {
  ...
  // Regra solicitada: 2kg por semana
  const kgPerWeek = 2;
...
form.addEventListener("submit", (e) => {
  ...
  if (!gender || !Number.isFinite(height) || !Number.isFinite(weight) || height <= 0 || weight <= 0) {
    errorEl.textContent = "Preencha todos os campos com valores válidos.";
    return;
  }
  ...
  idealLine.innerHTML = `Peso ideal: <strong>${idealRounded} kg</strong>`;
